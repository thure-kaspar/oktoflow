...
import de.iip_ecosphere.platform.support.aas.aas.NoOpAasServer;
...
public class BaSyxDeploymentRecipe implements DeploymentRecipe {
     ...
     */
    public BaSyxDeploymentRecipe(SetupSpec spec) {
        this.setupSpec = spec;
    }
    ...
    protected AasServer createImmediateDeploymentServer(SetupSpec spec, List<IORunnable> actions, String... options) {
        return new NoOpAasServer(); // client execution, does not need server; override in server
    }
    ...
    private class BaSyxImmediateDeploymentRecipe implements ImmediateDeploymentRecipe {
        ...
        public AasServer createServer(String... options) {
            return createImmediateDeploymentServer(setupSpec, actions, options);
        }
    ...
    private class BaSyxRegistryDeploymentRecipe implements RegistryDeploymentRecipe {
        ...
        @Override
        public AasServer createServer(String... options) {
            return createRegistryDeploymentServer(spec, options);
        }
     ...
     */
    public static void deploy(SetupSpec spec, Aas aas) throws IOException {
