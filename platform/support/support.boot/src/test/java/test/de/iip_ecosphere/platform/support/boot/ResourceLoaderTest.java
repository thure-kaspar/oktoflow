...
package test.de.iip_ecosphere.platform.support.boot;
...
public class ResourceLoaderTest {
    ...
    public void testResolver() throws IOException {
        ...
        Assert.assertFalse(ResourceLoader.knowsResourceResolver(res));
        ResourceLoader.registerResourceResolver(res);
        Assert.assertTrue(ResourceLoader.knowsResourceResolver(res));
        ...
        Assert.assertFalse(ResourceLoader.hasFilters());
        ResourceLoader.addTestExcludeFilters(); // identityStore in -test jars, shall not cause issues
        Assert.assertTrue(ResourceLoader.hasFilters());
        is = ResourceLoader.MAVEN_RESOLVER.resolve("Logo.jpg");
        Assert.assertNotNull(is);
        is.close();
        
        ResourceLoader.addFilter(f -> !f.toString().contains("/services/")); // REQUIRES CLASSPATH!!!
        is = ResourceLoader.getResourceAsStream("META-INF/services/" + ServiceLoaderUtilsDescriptor.class.getName());
        Assert.assertNull(is);
    ...
    /**
     *  Tests {@link OktoflowResourceResolver}.
     */
    @Test
    public void testOktoflowResourceResolver() throws IOException {
        ...
        // okto2 contains both folders with files, resources.ipr shall have precedence
        res = new OktoflowResourceResolver(new File("src/test/resources/okto2"));
        in = ResourceLoader.getResourceAsStream("nameplate.txt", res);
        Assert.assertNotNull(in);
        txt = Commons.getInstance().toString(in, Charset.defaultCharset()); // implemented
        Assert.assertEquals("resources.ipr", txt);
        in.close();

        // okto2 contains both folders with subfolders "software" and files, resources.ipr shall have precedence
        res = new OktoflowResourceResolver(new File("src/test/resources/okto3"), "software");
        in = ResourceLoader.getResourceAsStream(ResourceLoaderTest.class, "nameplate.txt", res);
        Assert.assertNotNull(in);
        txt = Commons.getInstance().toString(in, Charset.defaultCharset()); // implemented
        Assert.assertEquals("resources.ipr", txt);
        in.close();
    ...
    public void testResolvers() throws IOException {
        Assert.assertTrue(MultiResourceResolver.SETUP_RESOLVER.getName().length() > 0);
        
        // no src/main/resources -> okto3
        MavenResourceResolver mRes = new MavenResourceResolver(new File("src/test/resources/okto3"));
        Assert.assertTrue(mRes.getName().length() > 0);
        ...
        Assert.assertTrue(fRes.getName().length() > 0);
        in = fRes.resolve("prefix_nameplate.test"); // shall find "nameplate.test"
        Assert.assertNotNull(in);
        in.close();
    }
